      ...
      raw(
        sorted_keys.collect do |root_comment_key|
          comments[root_comment_key].collect do |comment|
            content_tag(:div, :class => (comment.id == root_comment_key ? (is_new ? "root_comment new_comments_background" : "root_comment") : (is_new ? "comment_reply new_comments_background" : "comment_reply")), :id => ("comment_#{comment.id}" if comment.id == root_comment_key) ) do
